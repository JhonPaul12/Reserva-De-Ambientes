...
export const CancelarS = () => {
  ...
  const [solicitudIdToDelete, setSolicitudIdToDelete] = useState<number | null>(null);
  ...
  const handleCancelarSolicitud = async (solicitudId: number) => {
    setSolicitudIdToDelete(solicitudId);
    setModalOpen(true);
  };

  const confirmarCancelarSolicitud = async () => {
    if (solicitudIdToDelete) {
      try {
        await axios.delete(`http://127.0.0.1:8000/api/solicitud/${solicitudIdToDelete}`);
        const nuevasSolicitudes = solicitudes.filter(
          (solicitud) => solicitud.id !== solicitudIdToDelete
        );
    ...
    <div className="contenedor-table">
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} className="p-10 bg-white">
        <ModalContent className="">
          ...
          <Button className="bg-danger m-2 text-white" onClick={confirmarCancelarSolicitud}>
            Sí, cancelar
          </Button>
